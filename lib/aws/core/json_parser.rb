  ...
  module Core
    ...
    class JSONParser
      ...
      def translate values, rules = @rules
        values.inject({}) do |data,(name,value)|
          key = rules[name][:sym] || name
          data.merge(key => translate_value(value, rules[name]))
        end
      end
      ...
      def translate_value value, rule
        ...
        when rule[:type] == :hash then translate(value, rule[:members])
        when rule[:type] == :map
          value.inject({}) do |data,(key,v)|
            data.merge(key => translate_value(v, rule[:members]))
          end
